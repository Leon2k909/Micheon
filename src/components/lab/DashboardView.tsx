    ...
    <section className="card flex flex-col justify-between overflow-hidden p-5 sm:p-6">
      <div>
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-black tracking-tight text-[var(--text-1)]">{ui(uiIsGerman() ? "Today's English progress" : "Today's German progress")}</h2>
            <p className="mt-1 text-sm font-semibold text-[var(--text-3)]">{ui("Your next useful step, streak, and practice totals.")}</p>
          </div>
        ...
        <div className="mt-5 grid items-center gap-5 rounded-[26px] bg-[var(--surface-2)] p-5 sm:grid-cols-[150px_minmax(0,1fr)]">
          <div className="flex min-h-[132px] flex-col justify-center border-b border-[var(--border)] pb-5 sm:border-b-0 sm:border-r sm:pb-0 sm:pr-5">
            <div className="flex items-center gap-2 text-[var(--text-3)]">
              ...
              <span className="text-[10px] font-black">{ui("A1-A2 path")}</span>
            </div>
            ...
            <p className="mt-2 text-xs font-bold text-[var(--text-3)]">{ui("Course complete")}</p>
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-black text-[var(--text-3)]">{ui("Next up")}</p>
            <h3 className="mt-1 truncate text-2xl font-black tracking-tight text-[var(--text-1)]">{uiOr(nextLessonTitle, "Konversationsmodul")}</h3>
            <p className="mt-2 line-clamp-2 text-sm leading-6 text-[var(--text-2)]">
              {uiOr(nextLesson?.focus ?? "Build the basics you need for everyday German conversations.", "Praktische Sätze für natürliche Alltagsgespräche.")}
            </p>
            ...
            <div className="mt-4 inline-flex items-center gap-2 rounded-full bg-[var(--surface)] px-3 py-2 text-xs font-black text-[var(--text-2)]">
              ...
              {ui("Finish one lesson to move today forward")}
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="card course-feature-card flex flex-col justify-between p-5 sm:p-6">
      <div>
        ...
        <h2 className="course-feature-title mt-4 text-2xl font-black leading-tight tracking-tight">
          {uiOr(currentPart?.theme ?? "German conversation basics", "Grundlagen für Gespräche")}
        </h2>
        <p className="course-feature-copy mt-2 max-w-md text-sm leading-6">
          {uiOr(currentPart?.description ?? "Build useful German for greetings, small talk, travel, and everyday questions.", "Praktische Sätze und Wörter für natürliche Alltagsgespräche.")}
        </p>
      </div>
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-3 line-clamp-2 text-lg font-black leading-tight">{uiOr(part.theme, "Konversationsmodul")}</h3>
        </div>
      ...
      <p className={active ? "mt-4 line-clamp-2 text-sm font-semibold leading-5 text-white/78" : "mt-4 line-clamp-2 text-sm font-semibold leading-5 text-[var(--text-2)]"}>
        {uiOr(part.focus || part.description || "Build practical German you can use today.", "Praktische Sätze für natürliche Alltagsgespräche").replace(/\.$/, "")}
      </p>
      ...
      <div className="mt-auto pt-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-black">{ui(active ? "Start daily lesson" : "Queued lesson")}</p>
              <p className={active ? "text-[11px] text-white/70" : "text-[11px] text-[var(--text-3)]"}>{Math.max(6, part.vocab.length)} {ui("words inside")}</p>
            </div>
          </div>
  ...
  return (
    ...
    >
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto]">
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-[var(--text-3)]">
                {totalWords.toLocaleString()} {ui("words available, including")} {wordBankWords.toLocaleString()} {ui("from the word bank.")}
              </p>
              <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-[var(--surface-2)] px-3 py-2 text-xs font-bold text-[var(--text-2)]">
                ...
                <span className="font-black text-[var(--text-1)]">{ui("Daily target:")}</span>
                <span>1 {ui("lesson")} + 5 {ui("words")}</span>
              </div>
            </div>
      ...
      <section className="grid gap-4 md:col-span-2 md:grid-cols-3">
        {[
          { icon: Target, label: "Conversation basics", value: `${coreParts.length} ${ui("modules")}`, tab: "learn" },
