...
export function buildSession(part: any, studyItems: any[], reviewState: any, _reviewStep: number) {
  ...
  const addSentence = (de: string, en: string, id: string, aliases: string[] = [], fr?: string, use?: string, lookup?: string, short?: string, when?: string, say?: string) => {
    ...
    if (rec?.lastGrade === "know") {
      ...
      queue.push({ type: EX.SENTENCE, review: true, overdue: overdueBy(rec), interval: rec.intervalDays ?? 1, item: { id, de, en, fr, use, lookup, tierNote, short, when, say } });
      return;                                            // due — back in as a review
    }
    queue.push({ type: EX.SENTENCE, item: { id, de, en, fr, use, lookup, tierNote, short, when, say } });
  };
  ...
  phrases.forEach((ph, i) => {
    ...
    addSentence(ph.de, ph.en, `${partKey}-phrase-${i}`, [], ph.fr, ph.use, undefined, ph.short, ph.when, ph.say);
  });
