...
check("header reward art is framed as a consistent Micheon stat icon", prototype.includes("np-stat-chip__art np-stat-chip__art--${kind}") && /\.np-stat-chip__art\s*\{[^}]*place-items:\s*center;[^}]*border-radius:\s*14px;/s.test(styles) && /\.np-stat-chip__art \.np-reward-icon\s*\{[^}]*width:\s*38px;[^}]*height:\s*38px;/s.test(styles));
