...
check("the prototype guided lesson uses the neutral app canvas without a green wash", guidedCanvas.includes("--fs-bg: #fffaf1;") && guidedCanvas.includes("linear-gradient(180deg, #fbfaf6 0%, #f6f2e9 100%)") && !guidedCanvas.includes("radial-gradient") && !appStyles.includes(".prototype-guided-session::before"));
...
check("the guided replay tile has room for its tactile shadow", /prototype-guided-session \.fs-listen\s*\{[^}]*min-height:\s*56px;[^}]*padding:\s*6px 15px 10px 6px;/s.test(appStyles) && /prototype-guided-session :is\([\s\S]*?\.fs-listen \.fs-listen-icon,[\s\S]*?box-shadow:[\s\S]*?0 3px 0 #248831,[\s\S]*?0 9px 18px rgba\(42, 137, 53, 0\.16\);/s.test(appStyles));
